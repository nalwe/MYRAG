...
def org_add_user(request):
    ...
    # 🌍 Superuser: can choose org explicitly
    ...
    # =================================================
    # 🔒 This enforces SINGLE-ORG membership
    users = (
        User.objects
        .select_related("profile")
        .filter(profile__organization__isnull=True)
        .order_by("email")
    )
    ...
    if request.method == "POST":
        ...
        # -------------------------------------------------
        # 🔒 HARD SAFETY CHECK (no multi-org users)
        # -------------------------------------------------
        ...
        # Only superusers can create org admins
        if request_user.is_superuser:
